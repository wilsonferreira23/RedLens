import importlib.util
import json
import os
import subprocess
import sys
import tempfile
import unittest
from pathlib import Path
from unittest import mock


ROOT = Path(__file__).resolve().parents[1]
CTL = ROOT / "engine" / "redlensctl.py"
ADAPTER_PATH = ROOT / "adapters" / "param_discovery.py"
SPEC = importlib.util.spec_from_file_location("param_discovery", ADAPTER_PATH)
ADAPTER = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(ADAPTER)


class ParamDiscoveryTest(unittest.TestCase):
    def setUp(self):
        self.temp = tempfile.TemporaryDirectory()
        self.env = {**os.environ, "REDLENS_HOME": self.temp.name}
        created = self._call_ctl("init", "--target", "https://app.example.test",
            "--authorized")
        self.run_id = created["run_id"]
        self.directory = Path(created["directory"])

    def tearDown(self):
        self.temp.cleanup()

    def _call_ctl(self, *args):
        result = subprocess.run([sys.executable, str(CTL), *args],
                                env=self.env, text=True, capture_output=True)
        if result.returncode:
            self.fail(result.stderr)
        return json.loads(result.stdout)

    def _urlopen_side_effect(self, request, *args, **kwargs):
        url = request.full_url
        body = b"baseline"
        status = 200
        if "id=" in url:
            body = b"user profile"
        elif "debug=" in url:
            body = b"debug info"
        mock_response = mock.MagicMock()
        mock_response.status = status
        mock_response.read.return_value = body
        mock_response.__enter__.return_value = mock_response
        mock_response.__exit__.return_value = False
        return mock_response

    def test_discovers_hidden_parameters(self):
        with mock.patch.object(ADAPTER.redlensctl, "RUNS", self.directory.parent):
            with mock.patch.object(ADAPTER.urllib.request, "urlopen", side_effect=self._urlopen_side_effect):
                result = ADAPTER.analyze(self.run_id, "https://app.example.test/search")

        self.assertTrue(result["ok"])
        self.assertEqual(result["discovered"], 2)

        summary = json.loads((self.directory / result["summary"]).read_text(encoding="utf-8"))
        params = {item["param"] for item in summary["discovered"]}
        self.assertEqual(params, {"id", "debug"})

        inventory = {
            json.loads(p.read_text(encoding="utf-8"))["value"]
            for p in (self.directory / "inventory").glob("*.json")
        }
        self.assertIn("GET https://app.example.test/search?id", inventory)
        self.assertIn("GET https://app.example.test/search?debug", inventory)

        coverage = json.loads((self.directory / "state" / "coverage.json").read_text(encoding="utf-8"))
        self.assertEqual(coverage["categories"]["api"]["status"], "confirmed")


if __name__ == "__main__":
    unittest.main()
